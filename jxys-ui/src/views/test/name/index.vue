...
<script>
import { listName, getName, delName, addName, updateName } from "@/api/test/name";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除测试名称编号为"' + ids + '"的数据项？').then(function() {
        return delName(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('test/name/export', {
        ...this.queryParams
